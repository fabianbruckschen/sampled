...
# generate additional independent masks which have a multiplicative relation
...
def mul(x, y, Q=None):
    ...
    # create z with additive relationship (using indicator function to only add
    # alpha*beta once)
    z = [(st[i] + (alpha * t[i]) + (beta * s[i]) + int(i == 0)*(alpha*beta))
         % Q for i in range(n_shares)]
